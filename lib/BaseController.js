    ...
    _.extend(this, Backbone.Events, {
        ...
        setParent: function (parent) {
            ...
            if (parent.__iamalloy) {
                ...
                this.parent = parent.getView('container') || parent.parent || parent;
                if (this.parent.__iamalloy) {
                    this.parent = this.parent.getViewEx({ recurse: true });
                }
	            //end-tiger8-hack
            } else {
                this.parent = parent;
        ...
        updateViewsDeep: function (args) {
            var views = this.getViews();
            if (_.isObject(args)) {
                
                _.each(views, function (view) {
                    if (typeof view.updateViewsDeep === 'function') {
                        view.updateViewsDeep(args);
                    }
                    else if (typeof view.applyProperties === 'function') {
                        var props = args['#' + view.id];
                        if (_.isObject(props)) {
		        ...
		        //_.each(_.keys(args), function(key) {
			       ...
			       // if (key.indexOf('#') === 0 && key !== '#' && _.isObject(elem) && typeof elem.applyProperties === 'function') {
				      //  // apply the properties but make sure we're applying them to a Ti.UI object (not a controller)
				      //  elem.applyProperties(args[key]);
			       // } else if (key.indexOf('#') === 0 && key !== '#' && _.isObject(elem) && typeof elem.updateViewsDeep === 'function') {
				      //  elem.updateViewsDeep(args);
			       // }
		        //});
            }
            return this;
