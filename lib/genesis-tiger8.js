...
var preCompile = function (alloyConfig, alloyLogger, config) {
    ...
    deleteCustomViewTargetFolders(alloyConfig, alloyLogger, config);
...
function findCustomViews(alloyConfig, alloyLogger, config) {
    ...
    deleteCustomViewSourceFolders(alloyConfig, alloyLogger, config);
 ...
 * @param {object} config Genesis config
 */
function deleteCustomViewTargetFolders(alloyConfig, alloyLogger, config) {
    
    var deletePaths = ['lib', 'views', 'controllers', 'styles', 'models', 'assets', 'config', 'migrations', 'themes', 'vendor', 'widgets'];
    
    config.customFolders.forEach(function (viewFolderName) {
        var newViewFolderName = viewFolderName.replace(/[_]+(\S+)/gi, '$1');
        deletePaths.forEach(function (deletePath) {
            var targetPath = path.join(alloyConfig.dir[deletePath], newViewFolderName);
            if (fs.existsSync(targetPath) && fs.statSync(targetPath).isDirectory()) {
                alloyLogger.warn('Deleting custom target path: ' + targetPath);
                fs.deleteDirSync(targetPath);
            }
        });
    });
 ...
 * @param {object} config Genesis config
 */
function deleteCustomViewSourceFolders(alloyConfig, alloyLogger, config) {
    var deletePaths = [];
